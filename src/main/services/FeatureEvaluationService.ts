...
class FeatureEvaluationService {
  ...
  > {
    ...
    > = getFeatureEvaluationExpressionsByService(userPricings, options.server ?? false);
    ...
    const evaluationResults = evaluateAllFeatures(
      pricingContext, 
      subscriptionContext, 
      evaluationContext, 
      !options.details
    ...
    // Return appropriate response based on options
